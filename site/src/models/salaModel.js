...
function listar(idAndar) {
    ...
    left join (select id_computador, count(*) as total_computador, fk_sala from andar join sala on andar.id_andar = sala.fk_andar join computador on sala.id_sala = computador.fk_sala where sala.fk_andar = ${idAndar} group by fk_sala) as comp on sa.id_sala = comp.fk_sala
    left join (select id_computador, count(*) as total_computador, fk_sala from andar join sala on andar.id_andar = sala.fk_andar join computador on sala.id_sala = computador.fk_sala WHERE andar.id_andar = ${idAndar} AND computador.is_ativo = 1 AND (computador.problema_cpu = 1 OR computador.problema_disco = 1 OR computador.problema_memoria = 1 OR computador.problema_fisico = 1)) as qtd_problema on sa.id_sala = comp.fk_sala
