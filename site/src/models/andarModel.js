...
function listar(idFaculdade) {
    ...
    left join (select id_sala, count(*) as total_sala, fk_andar from andar join sala on andar.id_andar = sala.fk_andar where andar.fk_faculdade = ${idFaculdade} group by fk_andar) as sa on andar.id_andar = sa.fk_andar
    ...
    left join (select id_computador, count(*) as total_computador, fk_sala from andar join sala on andar.id_andar = sala.fk_andar join computador on sala.id_sala = computador.fk_sala where andar.fk_faculdade = ${idFaculdade} AND computador.is_ativo = 1 AND (computador.problema_cpu = 1 OR computador.problema_disco = 1 OR computador.problema_memoria = 1 OR computador.problema_fisico = 1)) as qtd_problema on sa.id_sala = comp.fk_sala
